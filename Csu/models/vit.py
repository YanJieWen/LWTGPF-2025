...
class PatchEmbed(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, stride_size=16, in_chans=3, embed_dim=768, stem_conv=False):
        ...
        self.num_x = (img_size[1] - patch_size[1]) // stride_size_tuple[1] + 1 #向下取整
        self.num_y = (img_size[0] - patch_size[0]) // stride_size_tuple[0] + 1
        print('using stride: {}, and patch number is num_y{} * num_x{}'.format(stride_size, self.num_y, self.num_x))
        ...
        if self.stem_conv:
            hidden_dim = 64
            stem_stride = 2
            stride_size = patch_size = patch_size[0] // stem_stride #8
...
class TransReID(nn.Module):
    ...
    def forward_features(self, x, camera_id, view_id):
        ...
        if self.has_early_feature: #True
            for blk in self.blocks[:-1]:
                x = blk(x)
            if self.enable_early_norm:#False
                x = self.norm(x)
            return x
...
def vit_base_patch16_224_TransReID(img_size=(256, 128), stride_size=16, drop_path_rate=0.1, camera=0, view=0,
                                   has_early_feature=False, sie_xishu=1.5,
    ...
    model = TransReID(
        ...
        has_early_feature=has_early_feature, enable_early_norm=enable_early_norm, **kwargs)
...
def vit_small_patch16_224_TransReID(img_size=(256, 128), stride_size=16, drop_path_rate=0.1, camera=0, view=0,
    ...
    model = TransReID(
        ...
        camera=camera, view=view, sie_coef=sie_xishu, has_early_feature=has_early_feature,
        enable_early_norm=enable_early_norm, **kwargs)
    model.in_planes = 384
