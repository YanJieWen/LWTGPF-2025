...
class PartMatMul(Function):
    ...
    def forward(ctx, inputs, targets, em, alpha):
        ...
        outputs = inputs.matmul(
            ctx.em.permute(0, 2, 1))  # (n_part, b, c) x (n_part, c, n_proxy) -> (n_part, b, n_proxy)
        return outputs
    ...
    def backward(ctx, grad_outputs):
        ...
        for i, y in enumerate(targets):#利用样本迭代更新代理记忆
            x = inputs[:, i, :]  # (n_part, c)
            #伪标签的更新方式
...
class MultiPartMemory(nn.Module):
    def __init__(self, cfg):
        ...
        self.temp = cfg['proxy_temp']
        self.momentum = cfg['momentum']
        self.neg_k = cfg['bg_knn']
        self.posK = cfg['pos_k']
        self.balance_w = cfg['balance_w']
        self.part_weight = cfg['part_w']
    ...
    def forward(self, feature_dict, targets, cam=None, epoch=None):
        ...
        # print(self.all_proxy_labels.device,targets.device,self.proxy_memory.device,self.unique_cams.device)
        batch_proxy_labels = self.all_proxy_labels[targets.to('cpu')].to(self.device)#proxy in the cpu for concat

        # loss computation
        all_feats = torch.cat([feature_dict['global'].unsqueeze(0), feature_dict['part']], dim=0)
        all_scores = part_matmul(all_feats, batch_proxy_labels, self.proxy_memory,
                                 self.momentum)  # (n_part, b, n_proxy)
        ...
        global_on_loss, part_on_loss = self.online_loss_part_parallel(all_scaled_scores, batch_proxy_labels,
                                                                      all_temp_scores)
    ...
    def offline_loss_part_parallel(self, scores: torch.Tensor, labels: torch.Tensor) -> torch.Tensor:
        ...
        for i in range(scores.size(1)): #scan per sample
            ...
            loss += logit[0, :].sum()#glob feature part
    ...
    def online_loss_part_parallel(self, scores: torch.Tensor, labels: torch.Tensor, temp_scores: torch.Tensor):
        ...
        all_cam_tops = []#与相机最匹配的代理索引
        for cc in self.unique_cams:#选择每个摄像头下最近邻代理
            proxy_inds = self.cam2proxy[int(cc)].long().to(self.device)  # 当前相机下对应的多个proxy label
            max_idx = sims[:, :, proxy_inds].argmax(dim=2) #与相机cc最临近的proxy索引
        ...
        scatter_sims = torch.scatter(sims, dim=2, index=pos_inds,
                                     src=10000.0 * torch.ones(pos_inds.shape).type_as(sims))  # (1+N_part, B, N_proxy)
        ...
        # global loss-->B*NP-->B-->mean(B)
        loss = -1.0 * (F.log_softmax(sel_inputs[0], dim=1) * sel_targets[0]).sum(dim=1).mean()  # scalar

        # part loss
        if scores.size(0) > 1:
            part_loss = -1.0 * (F.log_softmax(sel_inputs[1:], dim=2) * sel_targets[1:]).sum(dim=2).mean(
                dim=1)  # (N_part, )
        else:
