...
from .vit import vit_small_patch16_224_TransReID
...
class TMGF(nn.Module):
    ...
    def __init__(self, arch, img_size, sie_coef, camera_num, view_num, stride_size, drop_path_rate, drop_rate,
                 attn_drop_rate,
                 pretrain_path, hw_ratio, gem_pool, stem_conv, num_parts, has_early_feature, has_head,
                 global_feature_type,
        ...
        self.base = TMGF.__factory[arch](img_size=img_size, sie_xishu=sie_coef, camera=camera_num, view=view_num,
                                         ...
                                         enable_early_norm=enable_early_norm,
                                         **kwargs)  # local_feature = True for no projection head ablation
        ...
        # Pooling layers-->
        for i, g in enumerate(self.granularities): #将高度变为g，w边为1
    ...
    def forward_multi_branch(self, x, label=None, cam_label=None, view_label=None):
        ...
        x_b1_patch = self.b1_pool(x_b1_patch).squeeze(-1)  # (B, C, P1)
        ...
        x_b2_patch = self.b2_pool(x_b2_patch).squeeze(-1)  # (B, C, P2)
        ...
        # print(x_b1_patch.shape,x_b2_patch.shape)
